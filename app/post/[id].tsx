...
export default function PostDetailScreenInner() {
            ...
            onPress: async () => {
              try {
                ...
                setPost(prevPost => {
                  ...
                  return {
                    ...prevPost,
                    answers: prevPost.answers.filter(
  ...
  const fetchPost = useCallback(
    async (questionId: string, retryCount: number = 0) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const submitReply = async () => {
    ...
    try {
      ...
      setRepliesByAnswer(prev => {
        const currentReplies = prev[replyingTo.id] || {
          items: [],
          loading: false,
          error: null,
          expanded: true,
        };
        const existingItems = Array.isArray(currentReplies.items)
  ...
  const renderCommentNode = useCallback(
    (node: ForumCommentNode, depth = 0) => (
      <View
        key={node.id}
        style={[styles.replyItem, depth > 0 && { marginLeft: depth * 12 }]}
      >
        ...
        <View key={answer.id} style={styles.answerCard}>
          ...
          {replyState?.expanded && (
            <View style={styles.repliesContainer}>
              ...
              {!replyState.loading &&
                !replyState.error &&
                Array.isArray(replyState.items) &&
              ...
              {replyingTo?.id === answer.id && (
                <View style={styles.replyComposer}>
                  ...
                  <TouchableOpacity
                    ...
                    style={[
                      styles.sendBtn,
                      (!replyBody.trim() || submittingReply) && {
                        opacity: 0.6,
                      },
                    ]}
                  >
  ...
  if (error) {
    return (
      <View style={styles.center}>
        ...
        <TouchableOpacity
          onPress={() => router.replace(`/post/${id}`)}
              ...
              <View style={styles.answerActions}>
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!newAnswerBody.trim() || submittingAnswer) &&
